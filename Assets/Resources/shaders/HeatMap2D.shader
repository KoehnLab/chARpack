		...
		SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				{
					diff = IN.texcoord - (_Points[i].xy * normalization);
					diff = diff * inv_aspect;

					norm = diff.x * diff.x + diff.y * diff.y;
					heat += saturate(_Points[i].z * _Intensity * exp(-norm / _Radius));
				}
